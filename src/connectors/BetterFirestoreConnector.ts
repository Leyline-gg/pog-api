...
const _initialize = function initializeDataSource(
  ...
  dataSource.connector = new BetterFirestoreConnector(dataSource.settings!);
  process.nextTick(() => {
    callback();
  });
};

class BetterFirestoreConnector extends Connector {
  public _models: any;
  public db: any;
  ...
  private addInnerFiltersToQuery = (query: Query, key: string, value: any) => {
    ...
    for (const operation in value) {
      ...
      resultQuery = resultQuery.where(key, '==', comparison);
    }
